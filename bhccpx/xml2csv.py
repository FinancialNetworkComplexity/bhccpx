...
def write_head(config: ConfigParser, outfile: TextIOWrapper, template: list[str]):
...
def write_elem(config: ConfigParser, elem: str, outfile: TextIOWrapper, template: list[str]):
	...
	keyvals: dict[str, str | None] = {key: None for key in template}
	...
	outfile.write(delim.join(['' if keyvals[k] is None else str(keyvals[k]) for k in template]) + '\n')
...
def parse_nic_file(config: ConfigParser, xmlfilename: str):
	"""Parses a single NIC XML file and writes an equivalent CSV file."""
...
def parse_nic(config: ConfigParser, xmlfile_globs: list[str] | None = None):
	"""Parses NIC XML files. If xmlfiles is provided, parse those; otherwise use config defaults."""
	if not xmlfile_globs:
		xmlfile_globs = [
	...
	for xmlfile_glob in xmlfile_globs:
		if os.path.isabs(xmlfile_glob):
			xmlfile_glob_qualified = xmlfile_glob
		else:
			xmlfile_glob_qualified = os.path.join(config.get('DEFAULT', 'datadir'), xmlfile_glob)
		
		matches = glob.glob(xmlfile_glob_qualified)
		if not matches:
			logger.warning('XML file not found: %s', xmlfile_glob_qualified)
			continue
		
		for xmlfile in matches:
			parse_nic_file(config, xmlfile)
			logger.info('xml2csv conversion for file %s complete', xmlfile)


def process(config: ConfigParser, *xmlfiles: str):
	parse_nic(config, None if xmlfiles is None else list(xmlfiles))
