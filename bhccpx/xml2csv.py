...
def write_head(config, outfile, template):
    ...
    """
    LOG.debug('Entering function', extra={'src':'write_head.entering'})
    if ('<TAB>'==config['xml2csv']['delim']):
        delim = '\t'  
    else:
        delim = config['xml2csv']['delim']
    for k in template:
...
#    (verbose, veryverbose) = UTIL.verbosity(config)
    xmlfilepath = os.path.join(config['xml2csv']['indir'], xmlfilename)
...
#    if (veryverbose): print('chunk', chunk)
    infile.close()
    elemtype = ''
    ...
    LOG.warning('Opening for output: '+outfilepath, 
                extra={'src':'parse_nic_file.outfilepath0'})
    ...
    while not(infileEOF):
        ...
        if (chunk.upper().find(xml1)>=0):
            ...
            LOG.warning('Closing output file: '+ outfilepath, 
                extra={'src':'parse_nic_file.outfilepath1'})
            infile.close()
            LOG.warning('Closing input file: ', xmlfilepath, 
                extra={'src':'parse_nic_file.xmlfilepath1'})
    if (LOG.getEffectiveLevel()<logging.WARNING):
...
def parse_nic(config):
    ...
    LOG.warning('**** Processing complete ****', 
                extra={'src':'parse_nic.proc_complete'})
