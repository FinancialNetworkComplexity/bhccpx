...
def make_wachwells_comparison(config):
    ...
    if (verbose): 
        ...
        print('*** Processing Table2 Complete ***')
    return table2
...
def test_metrics(metrics, context):
    ...
    if (1 != metrics[M_BCmp]):
        print('ERROR: BHC is not completely connected. '+M_BCmp+': '+str(metrics[M_BCmp])+', Context: '+context)
    # Confirm that Equation 3 (Euler-Poincare) holds
    if (metrics[M_BCrk] != metrics[M_BEct] - metrics[M_BVct] + metrics[M_BCmp]):
        print('ERROR: Euler-Poincare fails. '+M_BCrk+': '+str(metrics[M_BCrk])+', '+M_BEct+': '+str(metrics[M_BEct])+', '+M_BVct+': '+str(metrics[M_BVct])+', '+M_BCmp+': '+str(metrics[M_BCmp])+', Context: '+context)
    ...
    if (metrics[M_EQfxB] != metrics[M_BCrk] + metrics[M_BVct] - metrics[M_ENlbl]):
        print('ERROR: Theorem 3.2 fails. '+M_EQfxB+': '+str(metrics[M_EQfxB])+', '+M_BCrk+': '+str(metrics[M_BCrk])+', '+M_BVct+': '+str(metrics[M_BVct])+', '+M_ENlbl+': '+str(metrics[M_ENlbl])+', Context: '+context)
    ...
    if (metrics[M_EQhxB] != metrics[M_EDHmM] - metrics[M_ENlbl] + metrics[M_BCrk] - metrics[M_EDHmB]):
        print('ERROR: Corollary 3.4 fails. '+M_EQhxB+': '+str(metrics[M_EQhxB])+', '+M_EDHmM+': '+str(metrics[M_EDHmM])+', '+M_ENlbl+': '+str(metrics[M_ENlbl])+', '+M_BCrk+': '+str(metrics[M_BCrk])+', '+M_EDHmB+': '+str(metrics[M_EDHmB])+', Context: '+context)
    ...
    if (metrics[M_GQfxB] != metrics[M_BCrk] + metrics[M_BVct] - metrics[M_GNlbl]):
        print('ERROR: Theorem 3.2 fails (Geographic quotient). '+M_GQfxB+': '+str(metrics[M_GQfxB])+', '+M_BCrk+': '+str(metrics[M_BCrk])+', '+M_BVct+': '+str(metrics[M_BVct])+', '+M_GNlbl+': '+str(metrics[M_GNlbl])+', Context: '+context)
...
def all_bhc_complex(config, asofdate):
    ...
    DATA = UTIL.makeDATA(config['bhc2out']['indir'], fA, fB, fC, fREL, asofdate)
    BankSys = csv2sys.make_banksys(config, asofdate)
    HHs = eval(config['bhc2out']['bhclist'])
    if (len(HHs) <= 0):
        HHs = DATA[UTIL.IDX_HighHolder]      # Include all RSSDs when HHs empty
...
def main(argv=None):
    ...
    MPanel = ('TRUE'==config['bhc2out']['make_panel'].upper())
    if (MPanel): make_panel(config)
    MWWcomp = ('TRUE'==config['bhc2out']['make_wachwells_comparison'].upper())
    if (MWWcomp): make_wachwells_comparison(config)
